...
TEST_SUITE(vccrypt_mock_cipher_key_agreement_functions);
...
TEST(dispose_mocked)
{
    ...
    TEST_EXPECT(&suite.key_cipher_opts == got_options);
...
TEST(keypair_create_default)
{
    ...
    TEST_EXPECT(
        VCCRYPT_ERROR_MOCK_NOT_ADDED
            == vccrypt_key_agreement_keypair_create(&agreement, &priv, &pub));
