...
TEST_SUITE(vccrypt_buffer_init);
...
TEST(simpletest)
{
    ...
    TEST_ASSERT(0 == vccrypt_buffer_init(&buffer, &alloc_opts, BUFFER_SIZE));
    //the buffer alloc opts should be set
    TEST_EXPECT(&alloc_opts == buffer.alloc_opts);
    //the size should be 4
    TEST_EXPECT(BUFFER_SIZE == buffer.size);
    ...
    TEST_EXPECT(mock_allocator_allocate_called(&alloc_opts, BUFFER_SIZE));
