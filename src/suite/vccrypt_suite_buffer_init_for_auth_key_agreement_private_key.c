/**
 * \file vccrypt_suite_buffer_init_for_auth_key_agreement_private_key.c
 *
    ...
    vccrypt_buffer_t* buffer)
{
    ...
    MODEL_ASSERT(options->key_auth_opts.private_key_size > 0);

    return vccrypt_buffer_init(
        buffer, options->alloc_opts,
        options->key_auth_opts.private_key_size);
}
