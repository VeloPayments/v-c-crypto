/**
 * \file vccrypt_suite_hash_init.c
 *
 * Initialize the hash algorithm for the given crypto suite.
 *
    ...
    vccrypt_suite_options_t* options, vccrypt_hash_context_t* context)
{
    ...
    MODEL_ASSERT(options->vccrypt_suite_hash_alg_init != NULL);

    return options->vccrypt_suite_hash_alg_init(options, context);
}
