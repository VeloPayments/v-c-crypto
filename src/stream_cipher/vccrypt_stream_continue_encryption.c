/**
 * \file vccrypt_stream_continue_encryption.c
 *
 * Generic encryption continue method for a stream cipher.
 *
...
/**
 * \brief Algorithm-specific continuation for the stream cipher encryption.
 *
    ...
    size_t input_offset)
{
    ...
    MODEL_ASSERT(NULL != context->options->vccrypt_stream_alg_continue_encryption);
    MODEL_ASSERT(NULL != iv);
    MODEL_ASSERT(0 < iv_size);
    MODEL_ASSERT(0 < input_offset);


    return context->options->vccrypt_stream_alg_continue_encryption(
