/**
 * \file vccrypt_key_agreement_register_mock_cipher.c
 *
...
/* forward decls */
static int vccrypt_mock_cipher_init(void* options, void* context);
static void vccrypt_mock_cipher_dispose(void* options, void* context);
static int vccrypt_mock_cipher_options_init(
    void* options, allocator_options_t* alloc_opts);
static void vccrypt_mock_cipher_options_dispose(void* disp);
static int vccrypt_mock_cipher_long_term_secret_create(
    void* context, const vccrypt_buffer_t* priv,
    const vccrypt_buffer_t* pub, vccrypt_buffer_t* shared);
static int vccrypt_mock_cipher_short_term_secret_create(
    void* context, const vccrypt_buffer_t* priv,
...
static int vccrypt_mock_cipher_keypair_create(
    void* context, vccrypt_buffer_t* priv, vccrypt_buffer_t* pub);

/* static data for this instance */
static abstract_factory_registration_t mock_cipher_impl;
static vccrypt_key_agreement_options_t mock_cipher_options;
static bool mock_cipher_impl_registered = false;

/**
 * Register mock_cipher for use by the crypto library.
 */
void vccrypt_key_agreement_register_mock_cipher()
{
    MODEL_ASSERT(!mock_cipher_impl_registered);

    /* only register once */
    if (mock_cipher_impl_registered)
    {
        return;
    }

    /* set up the options for mock_cipher */
    mock_cipher_options.hdr.dispose =
        &vccrypt_mock_cipher_options_dispose;
    mock_cipher_options.alloc_opts = 0; /* allocator handled by init */
    mock_cipher_options.prng_opts = 0; /* prng options handled by init */
    mock_cipher_options.hash_algorithm =
        VCCRYPT_HASH_ALGORITHM_SHA_2_512;
    mock_cipher_options.hmac_algorithm =
        VCCRYPT_MAC_ALGORITHM_SHA_2_512_HMAC;
    mock_cipher_options.shared_secret_size =
        VCCRYPT_KEY_AGREEMENT_CURVE25519_SHA512_SECRET_SIZE;
    mock_cipher_options.private_key_size =
        VCCRYPT_KEY_AGREEMENT_CURVE25519_SHA512_PRIVATE_KEY_SIZE;
    mock_cipher_options.public_key_size =
        VCCRYPT_KEY_AGREEMENT_CURVE25519_SHA512_PUBLIC_KEY_SIZE;
    mock_cipher_options.minimum_nonce_size =
        VCCRYPT_KEY_AGREEMENT_CURVE25519_SHA512_NONCE_SIZE;
    mock_cipher_options.vccrypt_key_agreement_alg_init =
        &vccrypt_mock_cipher_init;
    mock_cipher_options.vccrypt_key_agreement_alg_dispose =
        &vccrypt_mock_cipher_dispose;
    mock_cipher_options.vccrypt_key_agreement_alg_long_term_secret_create =
        &vccrypt_mock_cipher_long_term_secret_create;
    mock_cipher_options.vccrypt_key_agreement_alg_short_term_secret_create =
        &vccrypt_mock_cipher_short_term_secret_create;
    mock_cipher_options.vccrypt_key_agreement_alg_keypair_create =
        &vccrypt_mock_cipher_keypair_create;
    mock_cipher_options.vccrypt_key_agreement_alg_options_init =
        &vccrypt_mock_cipher_options_init;

    /* set up this registration for the abstract factory */
    mock_cipher_impl.interface = VCCRYPT_INTERFACE_KEY;
    mock_cipher_impl.implementation =
        VCCRYPT_KEY_AGREEMENT_ALGORITHM_MOCK_CIPHER;
    mock_cipher_impl.implementation_features =
        VCCRYPT_KEY_AGREEMENT_ALGORITHM_MOCK_CIPHER;
    mock_cipher_impl.factory = 0;
    mock_cipher_impl.context = &mock_cipher_options;

    /* register this instance */
    abstract_factory_register(&mock_cipher_impl);

    /* only register once */
    mock_cipher_impl_registered = true;
}
 ...
 *                  structure.
 */
static void vccrypt_mock_cipher_dispose(
